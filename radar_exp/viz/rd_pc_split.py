...
def visualize_split(
    ...
    for i, obj in enumerate(objs):
        ...
        ax_left.scatter(xs, ys, c=[colours[i]], s=60, alpha=0.8,
    ...
    if zoom_to_objects:
        ...
        if xs and ys:
            x_margin = (max(xs) - min(xs)) * 0.15 + 5.0
            y_margin = (max(ys) - min(ys)) * 0.15 + 5.0
            ax_left.set_xlim(min(xs) - x_margin, max(xs) + x_margin)
            ax_left.set_ylim(min(ys) - y_margin, max(ys) + y_margin)
    else:
        lim = max_range
    ...
    # Display RD map as image.  Use indices for coordinates and set extent
